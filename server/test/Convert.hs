...
fromPackageDescription Cabal.GenericPackageDescription{..} =
  ...
  Package { packageName = Cabal.unPackageName (Cabal.pkgName package)
      ...
      , packageLicense = Just (show (Cabal.disp Distribution.PackageDescription.license))
      , packageLicenseFile = listToMaybe licenseFiles
      , packageTestedWith =
          map toUpper .
          show .
          fsep . map (\(f, vr) -> Cabal.disp f <> Cabal.disp vr ) <$>
          nullNothing testedWith      , packageFlags =
          map (\Cabal.MkFlag{..} ->
...
fromPackageDescriptionString pkgBytes =
  case Cabal.parseGenericPackageDescription pkgBytes of
...
fromDependency :: Cabal.Dependency -> Dependencies
fromDependency (Cabal.Dependency pn vr)
  | vr == Cabal.anyVersion = Dependencies (show (Cabal.disp pn)) Nothing
  | otherwise              = Dependencies (show (Cabal.disp pn <+> Cabal.disp vr)) Nothing
...
sectionWithBuildInfo d Cabal.BuildInfo{..} =
  Section { sectionData = d
      ...
      , sectionDependencies = fromDependency targetBuildDepends
...
exeFromCondExecutableTup (name, Cabal.CondNode Cabal.Executable{..} _ _) =
  Executable { executableMain = modulePath
             , executableOtherModules = disp (Cabal.otherModules buildInfo)
             , executableGeneratedModules = disp (Cabal.autogenModules buildInfo)
             } where
    disp = map (show . Cabal.disp) 
...
testExeFromCondExecutableTup (name, Cabal.CondNode Cabal.TestSuite{..} _ _) =
  case testInterface of
    Cabal.TestSuiteExeV10 _ mainIs -> Just
      Executable { executableMain = mainIs
                 , executableOtherModules = disp (Cabal.otherModules testBuildInfo)
                 , executableGeneratedModules = disp (Cabal.autogenModules testBuildInfo)
                 } where
        disp = map (show . Cabal.disp) 
    _ -> Nothing
...
benchExeFromCondExecutableTup (name, Cabal.CondNode Cabal.Benchmark{..} _ _) =
  case benchmarkInterface of
    Cabal.BenchmarkExeV10 _ mainIs -> Just
      Executable { executableMain = mainIs
                 , executableOtherModules = disp (Cabal.otherModules benchmarkBuildInfo)
                 , executableGeneratedModules = disp (Cabal.autogenModules benchmarkBuildInfo)
                 } where        disp = map (show . Cabal.disp) 
    _ -> Nothing
...
splitOn _delim _str@"" = []
splitOn delimiter string = go "" string where
  go acc str = case stripPrefix delimiter str of
    Nothing       -> case str of
      (x:xs) -> go (x:acc) xs
      ""     -> [acc]
    Just stripped -> reverse acc : go "" stripped
